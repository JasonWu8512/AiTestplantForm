...
class DashboardStatisticsView(APIView):
    ...
    def get(self, request):
        ...
        # 获取测试用例状态分布
        testcase_status = testcase_queryset.values('status').annotate(count=Count('id'))
        
        # 获取测试计划状态分布
        testplan_status = testplan_queryset.values('status').annotate(count=Count('id'))
        
        # 获取测试结果状态分布
        testresult_status = testresult_queryset.values('status').annotate(count=Count('id'))
        
        # 获取测试用例优先级分布
        testcase_priority = testcase_queryset.values('priority').annotate(count=Count('id'))
